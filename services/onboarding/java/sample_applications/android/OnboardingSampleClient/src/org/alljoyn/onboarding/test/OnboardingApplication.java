...
/**
 * The OnboardingApplication class handles all the AllJoyn devices the application is announced on,
 * and enable the user to perform all the AllJoyn onboarding service methods. (get the scan information, do onboarding, offboarding etc.)
 */
public class OnboardingApplication extends Application implements AuthPasswordHandler
{
	...
	public void doConnect(){
		...
		m_logger.debug(TAG, "Setting daemon password");
		Status pasStatus = PasswordManager.setCredentials("ALLJOYN_PIN_KEYX", "000000");
	
		if ( pasStatus != Status.OK ) {
			m_logger.error(TAG, "Failed to set password for daemon, Error: " + pasStatus);
		}
	 ...
	 * @param networkAuthType The auth type of the configured network (can get by AuthType.getTypeId()).
	 */
	...
	//======================================================================
	/**
	 * Sets a password on the client side. This password will be compared against the alljoyn device password when needed. 
