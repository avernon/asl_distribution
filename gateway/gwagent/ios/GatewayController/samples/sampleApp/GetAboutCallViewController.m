...
- (void)UpdateCallViewInformation
{
    ...
    if (self.sessionId == 0 || self.sessionId == -1) {
        NSLog(@"Failed to join session. sid = %u", self.sessionId);
        ...
        if (qStatus != ER_OK) {
            ...
            NSLog(@"AboutData: %@", [AJNAboutDataConverter aboutDataDictionaryToString:aboutData]);

            NSLog(@"objectDescriptions: %@", [AJNAboutDataConverter objectDescriptionsDictionaryToString:objDesc]);
